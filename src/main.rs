...
fn run() -> Result<()> {
    ...
    match cli.command {
        ...
        Some(Commands::Init { force, path }) => {
            cli_exec::handle_init_command(force, path)?;
        }
        Some(Commands::Snap { message, json }) => {
            cli_exec::handle_snap_command(message, json)?;
        }
        Some(Commands::Snaps { json }) => {
            cli_exec::handle_snaps_command(json)?;
        }
        Some(Commands::Show { snap_id, json }) => {
            cli_exec::handle_show_command(snap_id, json)?;
        }
        Some(Commands::Restore { snap_id, force }) => {
            cli_exec::handle_restore_command(snap_id, force)?;
        }

        Some(Commands::Diff { from, to, json }) => {
            cli_exec::handle_diff_command(from, to, json)?;
        }

        Some(Commands::Mv { from, to }) => {
            cli_exec::handle_mv_command(from, to)?;
        }
        ...
        Some(Commands::Bundle {
            ...
            let ws = Workspace::discover(&std::env::current_dir().context("get current dir")?)?;
            cli_exec::handle_bundle_command(&ws, scope, gate, publications, json)?;
        }
        Some(Commands::Promote {
            ...
            let ws = Workspace::discover(&std::env::current_dir().context("get current dir")?)?;
            cli_exec::handle_promote_command(&ws, bundle_id, to_gate, json)?;
        }
